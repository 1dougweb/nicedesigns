...
@section('title', '- Editar Projeto: ' . $clientProject->name)
@section('page-title', 'Editar Projeto: ' . $clientProject->name)

@section('content')
<!-- Header -->
<div class="flex items-center justify-between mb-8">
    <div>
        <h2 class="text-2xl font-bold text-white mb-2">Editar Projeto de Cliente</h2>
        <p class="text-gray-400">Atualize as informações do projeto: {{ $clientProject->name }}</p>
    </div>
    <div class="flex space-x-4">
        <a href="{{ route('admin.client-projects.show', $clientProject) }}" 
...
<div class="bg-gray-800/50 backdrop-blur-md rounded-3xl border border-gray-700/50 p-8">
    <form action="{{ route('admin.client-projects.update', $clientProject) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" 
                           name="name" 
                           id="name"
                           value="{{ old('name', $clientProject->name) }}"
                           required
                ...
                <div>
                    ...
                    <select name="user_id" 
                        ...
                        @foreach($clients as $client)
                            <option value="{{ $client->id }}" {{ old('user_id', $clientProject->user_id) == $client->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <textarea name="description" 
                          ...
                          class="w-full bg-gray-700/50 border border-gray-600 text-white placeholder-gray-400 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description', $clientProject->description) }}</textarea>
                @error('description')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="status" 
                        ...
                        <option value="planejamento" {{ old('status', $clientProject->status) === 'planejamento' ? 'selected' : '' }}>Planejamento</option>
                        <option value="em_desenvolvimento" {{ old('status', $clientProject->status) === 'em_desenvolvimento' ? 'selected' : '' }}>Em Desenvolvimento</option>
                        <option value="em_revisao" {{ old('status', $clientProject->status) === 'em_revisao' ? 'selected' : '' }}>Em Revisão</option>
                        <option value="aguardando_cliente" {{ old('status', $clientProject->status) === 'aguardando_cliente' ? 'selected' : '' }}>Aguardando Cliente</option>
                        <option value="concluido" {{ old('status', $clientProject->status) === 'concluido' ? 'selected' : '' }}>Concluído</option>
                        <option value="pausado" {{ old('status', $clientProject->status) === 'pausado' ? 'selected' : '' }}>Pausado</option>
                        <option value="cancelado" {{ old('status', $clientProject->status) === 'cancelado' ? 'selected' : '' }}>Cancelado</option>
                    </select>
                    @error('status')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="priority" 
                        ...
                        <option value="baixa" {{ old('priority', $clientProject->priority) === 'baixa' ? 'selected' : '' }}>Baixa</option>
                        <option value="normal" {{ old('priority', $clientProject->priority) === 'normal' ? 'selected' : '' }}>Normal</option>
                        <option value="alta" {{ old('priority', $clientProject->priority) === 'alta' ? 'selected' : '' }}>Alta</option>
                        <option value="urgente" {{ old('priority', $clientProject->priority) === 'urgente' ? 'selected' : '' }}>Urgente</option>
                    </select>
                    @error('priority')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="date" 
                           name="start_date" 
                           id="start_date"
                           value="{{ old('start_date', $clientProject->start_date?->format('Y-m-d')) }}"
                           required
                ...
                <div>
                    ...
                    <input type="date" 
                           name="due_date" 
                           id="due_date"
                           value="{{ old('due_date', $clientProject->due_date?->format('Y-m-d')) }}"
                           required
                ...
                <div>
                    ...
                    <input type="number" 
                           name="budget" 
                           id="budget"
                           value="{{ old('budget', $clientProject->budget) }}"
                           min="0"
                           step="0.01"
                           placeholder="0,00"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-4">
                        <input type="range" 
                               name="progress" 
                               id="progress"
                               value="{{ old('progress', $clientProject->progress_percentage) }}"
                               min="0"
                               max="100"
                               step="5"
                               class="flex-1 h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer">
                        <span id="progress-value" class="text-cyan-400 font-bold min-w-[3rem]">{{ old('progress', $clientProject->progress_percentage) }}%</span>
                    </div>
                    @error('progress')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="current_stage" 
                        ...
                        <option value="Planejamento" {{ old('current_stage', $clientProject->current_stage) === 'Planejamento' ? 'selected' : '' }}>Planejamento</option>
                        <option value="Design" {{ old('current_stage', $clientProject->current_stage) === 'Design' ? 'selected' : '' }}>Design</option>
                        <option value="Desenvolvimento" {{ old('current_stage', $clientProject->current_stage) === 'Desenvolvimento' ? 'selected' : '' }}>Desenvolvimento</option>
                        <option value="Testes" {{ old('current_stage', $clientProject->current_stage) === 'Testes' ? 'selected' : '' }}>Testes</option>
                        <option value="Deploy" {{ old('current_stage', $clientProject->current_stage) === 'Deploy' ? 'selected' : '' }}>Deploy</option>
                    </select>
                    @error('current_stage')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                @php
                    ...
                    if ($clientProject->stages && is_array($clientProject->stages)) {
                        $stagesText = implode("\n", array_column($clientProject->stages, 'name'));
                    }
                @endphp
                <textarea name="stages" 
                          ...
                          class="w-full bg-gray-700/50 border border-gray-600 text-white placeholder-gray-400 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent @error('stages') border-red-500 @enderror">{{ old('stages', $stagesText) }}</textarea>
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <textarea name="requirements" 
                          ...
                          class="w-full bg-gray-700/50 border border-gray-600 text-white placeholder-gray-400 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent @error('requirements') border-red-500 @enderror">{{ old('requirements', $clientProject->requirements) }}</textarea>
                @error('requirements')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                @php
                    $technologiesText = '';
                    if (is_array($clientProject->technologies)) {
                        $technologiesText = implode(', ', $clientProject->technologies);
                    } elseif (is_string($clientProject->technologies)) {
                        $technologiesText = $clientProject->technologies;
                    }
                @endphp
                <input type="text" 
                       name="technologies" 
                       id="technologies"
                       value="{{ old('technologies', $technologiesText) }}"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="url" 
                           name="project_url" 
                           id="project_url"
                           value="{{ old('project_url', $clientProject->project_url) }}"
                ...
                <div>
                    ...
                    <input type="url" 
                           name="repository_url" 
                           id="repository_url"
                           value="{{ old('repository_url', $clientProject->repository_url) }}"
            ...
            <div>
                ...
                <textarea name="notes" 
                          ...
                          class="w-full bg-gray-700/50 border border-gray-600 text-white placeholder-gray-400 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent @error('notes') border-red-500 @enderror">{{ old('notes', $clientProject->notes) }}</textarea>
                @error('notes')
                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
